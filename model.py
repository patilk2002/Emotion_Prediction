...
class CustomLSTM(tf.keras.layers.Layer):
    def __init__(self, units):
        ...
        self.units = units
        # Define state size as a class attribute
    ...
    def build(self, input_shape):
        ...
        self.reg = self.add_weight(shape=(self.units,),
                                                   initializer='zeros',
                                                   trainable=True)
        # self.reg2 = self.add_weight(shape=(self.units,),
        #                                            initializer='zeros',
        #                                            trainable=True)
...
# Replace LSTM layer with CustomLSTM
...
model1 = load_model("LSTM_Tab.h5")

# Predict the output
prediction = model1.predict([test_lstm, test_tabular])
